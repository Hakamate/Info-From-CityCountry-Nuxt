    ...
    <div class="">
      ...
      <div class="m-10 flex items-start flex-col font-bold">
        <div class="text-gray-900 text-xl" :class="this.globalData.capital ? 'block' : 'hidden'">
          Capital : <span class="text-gray-700 font-light">{{this.globalData.capital}}</span>
        </div>
        <div class="text-gray-900 text-xl" :class="this.globalData.region ? 'block' : 'hidden'">
          Région : <span class="text-gray-700 font-light">{{this.globalData.region}}</span>
        </div>
        <div class="text-gray-900 text-xl" :class="this.globalData.population ? 'block' : 'hidden'">
          Population : <span class="text-gray-700 font-light">{{Intl.NumberFormat().format(this.globalData.population)}}</span>
        </div>
        <div class="text-gray-900 text-xl" :class="this.globalData.callingCodes ? 'block' : 'hidden'">
          Numéro d'appel : <span class="text-gray-700 font-light">{{this.globalData.callingCodes}}</span>
        </div>
      </div>
    </div>
  </div>
